...
- `sidebars.ts` is hand-written per project, not a single blanket
  `{type: 'autogenerated', dirName: '.'}`. Each project gets one top-level category
  (see the `Framework` category for `tsvrc`) built from `{type: 'autogenerated', dirName: '<project>/<folder>'}`
  subtrees grouped as Getting started / How-to guides / Reference / Explanations. That
  grouping is what keeps Diátaxis categories visually separated in the sidebar. A new
  project needs its own top-level category added there the same way; it won't show up
  from folder presence alone. Each folder-level category's `label` lives in `sidebars.ts`
  itself, not in a `_category_.json`: `dirName` pointed straight at a leaf folder
  returns its files as a flat, unlabeled list, so the label has to come from the
  wrapping `{type: 'category', ...}` node instead. Ordering *within* an autogenerated
  subtree still comes from each page's `sidebar_position` frontmatter.
...
- **Docs content** (`docs/<project>/`): stays English-only, unversioned, exactly where
  ...
  files. Sidebar category labels live in `sidebars.ts` itself (see the Structure
  section above), not in a translatable `_category_.json`, so they render in English
  for every locale regardless of `i18n/<locale>/docusaurus-plugin-content-docs/current.json`.
  That's deliberate for now, since a half-translated sidebar over English content would be
  worse than an English one. Translating them means giving `sidebars.ts` locale-aware
  labels (e.g. via `docusaurus-plugin-content-docs`'s own i18n message extraction) once
  real doc translations exist to justify it.
...
- **Worked example**: `docs/tsvrc/intro.md` has a real Spanish translation at
  ...
  Verified in-browser: the translated page renders correctly, and links from it to
  still-untranslated pages fall back to English cleanly under the same `/es/` prefix.
- **Headings that other pages link to by anchor** need an explicit, language-stable id,
  or translating the heading text breaks the anchor. Docusaurus's MDX v3 syntax for this
  is a comment, not `{#id}` (that's the old, now-invalid MDX v1/v2 form and fails the
  build): `## Play Mode tests: TsPlayModeTestBase {/* #play-mode-tests-tsplaymodetestbase */}`
  (see `docs/tsvrc/testing/testing-your-world.md`). Keep the same id across every
  language's version of that heading.
