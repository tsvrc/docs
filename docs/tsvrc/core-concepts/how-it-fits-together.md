...
as constructs, which pools exist, which globals are exposed — and writes plain C# files
into your project. Nothing in this half ships in the built world; it only ever runs while
you're editing.

The **runtime half** lives under `Runtime/` and is ordinary UdonSharp: base classes like
`TsvrcBehaviour`, `Instance`, and `TsvrcMemory` that your world's compiled scripts actually
extend and call into. This is the part that runs for players.
...
hand-write them, and re-running codegen regenerates them from your configuration each time.

## Why a generation step exists at all

Two things a framework like this often wants — a container that can look up "give me the
instance of type X that's registered right now" and typed access to whichever concrete
subclasses a given project happens to define — are usually built with runtime reflection
or dependency injection. TsVRC does both at edit time instead: the codegen pass inspects
your project's own types and configuration while you're still in the editor, and bakes the
result into generated code that has direct references and calls, not lookups.
...
generated, not resolved at runtime, and that's why your own scripts extend a generated
shadow class (`TsBehaviour`, `TsInstance`, and so on) instead of the framework class
directly — the shadow is what carries your project's concrete types.
...
product) in **Tsvrc > Configure** arranges for the generated code to do. A behaviour that's
sitting in the scene but never registered anywhere never gets constructed, and any of its
`TsStart` logic never runs.
