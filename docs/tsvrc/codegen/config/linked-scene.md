...
among the loaded scenes, for any reason, including having been deleted (a strict superset of
`IsConfiguredButMissing`). The generator's own run logic treats this as a hard "do nothing":
once a scene has been explicitly linked, no other loaded scene is ever treated as a
legitimate substitute for it.
