...
get confused for each other — an ambiguous match resolves to "unknown" rather than guessing.
Rebuilt fresh at the start of every generation pass, never cached across passes.
...
Persists each module's last successfully resolved entries (name, type, namespace only —
never a live object reference) to a per-module JSON cache. This is what
