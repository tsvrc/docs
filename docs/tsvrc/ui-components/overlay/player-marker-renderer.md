...
virtual methods, deliberately, so a subclass that only overrides one of the two methods
still works: the other simply does nothing. A `PlayerPositionOverlay` with no `Renderer`
assigned is a separate, explicit case: `StartOverlay` and the tracking hooks all check for
`Renderer == null` themselves and refuse to run rather than relying on this class's own
safety for that.
