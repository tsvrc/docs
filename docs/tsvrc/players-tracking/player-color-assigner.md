...
instance size](https://wiki.vrchat.com/wiki/Special:MyLanguage/Instances) (80 is the
...
and deterministic, but it means the *n*th player to ever join any instance always gets
...
- Calling `GetColor` before `Initialize` (or before the owner's synced order has
  arrived) is safe. The palette-index modulo alone still produces a color, just not yet
  the session-shuffled one.
- The palette itself is a fixed, hand-tuned hue sweep (two passes around the color
  wheel at alternating saturation/value) chosen for maximum distinctness between
  adjacent slots, not a generated gradient. Changing its length changes the modulo
  divisor for every ID.
